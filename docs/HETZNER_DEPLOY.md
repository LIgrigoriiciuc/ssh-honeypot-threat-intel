...
systemctl restart ssh.socket
```
Verify before closing the working session, from a second terminal:
```bash
ssh -i ~/.ssh/hetzner_honeypot -p 2222 root@<server-ip>
```
...
tail -f honeypot.json # live log, Ctrl+C to stop watching without killing the process
```
## Cron - hourly log snapshots
`honeypot.json` grows via append and is never truncated, so a "snapshot" here means a timestamped copy, not a rotation.
```bash
mkdir -p /opt/honeypot/backups
crontab -e
```
Default crontab opens with a large `#`-commented explanation block (syntax reference, examples). Boilerplate, not something to edit or remove - active entries go on new lines below it.
Added line:
```
